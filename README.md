...
- **即時報價**：每 5 秒自動更新，支援台股、日股、韓股、港股、美股、歐股
...
- **雙層快取**：慢層（歷史資料 + metadata，每 10 分鐘刷新）+ 快層（即時報價，每 5 秒刷新）
- **報價 API**：使用 Yahoo Finance v8/finance/spark 批次端點，每批最多 20 個 ticker，101 支股票僅需 6 個並行請求
- **公假日保護**：非美股市場利用 `history_metadata.regularMarketTime` 偵測節假日，避免誤判開盤

### 前端架構

- 純 Vanilla JS，無框架相依
- 每 5 秒輪詢 `/api/stocks`，每 15 秒輪詢 `/api/indices`
- 支援依地區篩選、開盤市場優先排序
